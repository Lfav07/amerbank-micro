...
class UserServiceTest {
    ...
    void shouldRegisterAdmin() {
        ...
        UserRegisterRequest request =
                new UserRegisterRequest("ADMIN@EMAIL.COM", "password");
    ...
    void shouldLoginUser() {
        ...
        when(customerServiceClient.getCustomerIdByUserId(user.getId())).thenReturn(customerId);
    ...
    @Test
    @DisplayName("Should throw EmailAlreadyTakenException when DataIntegrityViolationException occurs during user registration")
    void shouldThrowEmailAlreadyTakenExceptionWhenDataIntegrityViolationDuringRegistration() {
    ...
    @Test
    @DisplayName("Should handle customer deleted event and delete user")
    void shouldHandleCustomerDeletedEventAndDeleteUser() {
        Long userId = 1L;
        CustomerDeletedEvent event = new CustomerDeletedEvent(userId);

        when(userRepository.existsById(userId)).thenReturn(true);

        userService.handleCustomerDeleted(event);

        verify(userRepository).deleteById(userId);
    }

    @Test
    @DisplayName("Should not delete user when customer deleted event references non-existing user")
    void shouldNotDeleteWhenCustomerDeletedEventReferencesNonExistingUser() {
        Long userId = 999L;
        CustomerDeletedEvent event = new CustomerDeletedEvent(userId);
